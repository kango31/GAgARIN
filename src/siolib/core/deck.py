...
class Deck(Component):
    ...
    def __init__(self, face_up=False, **properties):
        ...
        :param face_up: by default, all cards in this deck are considered as
        specified
        :type face_up: bool
        """
    ...
    def add(self, card, position="top"):
        ...
        if position == "top":
            ...
            raise ValueError("Unknown 'position' for Deck.add: {}"
                    .format(position))
        return self
    ...
    def search(self, filter, number=0):
        ...
        The card are filtered by the given filter function which is a function
        taking card as argument. By default, all cards matching the filter are
        drawn and returned but it is possible to limit the number of cards with
        number argument.

        :param filter: filter function for cards or string query
        :type filter: callable or str
